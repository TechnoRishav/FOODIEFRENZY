...
const ListItems = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const { data } = await axios.get('https://foodiefrenzy-backend-1h1g.onrender.com/api/items');
        setItems(data);
      } catch (err) {
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      await axios.delete(`https://foodiefrenzy-backend-1h1g.onrender.com/api/items/${itemId}`);
